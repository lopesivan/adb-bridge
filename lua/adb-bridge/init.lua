...
function M.setup(opts)
  ...
  command("AdbShell", function(o)
    M.shell(o.args)
  end, { nargs = "+", desc = "Executa adb shell via bridge nativo" })
  command("AdbDevices", function()
    M.devices()
  end, { desc = "Lista dispositivos" })
  command("AdbDevicesL", function()
    M.devices_l()
  end, { desc = "Lista dispositivos com detalhes" })
  command("AdbVersion", function()
    M.version()
  end, { desc = "Mostra versão do protocolo do adb server" })
  command("AdbState", function()
    M.get_state()
  end, { desc = "Mostra estado do device" })
  command("AdbRoot", function()
    M.root()
  end, { desc = "Reinicia adbd como root" })
  command("AdbUnroot", function()
    M.unroot()
  end, { desc = "Reinicia adbd sem root" })
  command("AdbRemount", function()
    M.remount()
  end, { desc = "Remonta partições do sistema" })
  command("AdbReboot", function(o)
    M.reboot(o.args)
  end, { nargs = "?", desc = "Reinicia o device [bootloader|recovery]" })
  command("AdbTcpip", function(o)
    M.tcpip(tonumber(o.args))
  end, { nargs = 1, desc = "Ativa adb TCP/IP na porta indicada" })

  command("AdbForward", function(o)
    if #o.fargs ~= 2 then
      vim.notify(
        "[adb-bridge] uso: AdbForward <local> <remote>",
        vim.log.levels.ERROR
      )
      return
    end
  ...
  command(
    ...
    function(o)
      if #o.fargs ~= 2 then
        vim.notify(
          "[adb-bridge] uso: AdbPush <local> <remote>",
          vim.log.levels.ERROR
        )
        return
      end
    ...
    { nargs = "+", complete = "file", desc = "Envia arquivo para o device" }
  )

  command("AdbPull", function(o)
    if #o.fargs ~= 2 then
      vim.notify(
        "[adb-bridge] uso: AdbPull <remote> <local>",
        vim.log.levels.ERROR
      )
      return
    end
  ...
  command("AdbInstall", function(o)
    M.install(o.args)
  end, {
    nargs = 1,
    complete = "file",
    desc = "Instala APK usando push + pm install",
  })

  command("AdbInput", function(o)
    M.input(o.args)
  end, { nargs = "+", desc = "Executa input" })
  command("AdbSettings", function(o)
    M.settings(o.args)
  end, { nargs = "+", desc = "Executa settings" })
  command("AdbUiAutomator", function(o)
    M.uiautomator(o.args)
  end, { nargs = "+", desc = "Executa uiautomator" })
  command("AdbAm", function(o)
    M.am(o.args)
  end, { nargs = "+", desc = "Executa Activity Manager" })
  command("AdbPm", function(o)
    M.pm(o.args)
  end, { nargs = "+", desc = "Executa Package Manager" })
  command("AdbStart", function(o)
    M.start(o.args)
  end, { nargs = "+", desc = "Inicia uma Activity" })
  ...
  command("AdbKeyevent", function(o)
    M.keyevent(o.args)
  end, { nargs = 1, desc = "Envia keyevent" })

  command("AdbSwipe", function(o)
    if #o.fargs < 4 or #o.fargs > 5 then
      vim.notify(
        "[adb-bridge] uso: AdbSwipe <x1> <y1> <x2> <y2> [ms]",
        vim.log.levels.ERROR
      )
      return
    end
  ...
  command("AdbText", function(o)
    M.text(o.args)
  end, { nargs = "+", desc = "Digita texto" })
  command("AdbUninstall", function(o)
    M.uninstall(o.args)
  end, { nargs = "+", desc = "Desinstala pacote" })
end
...
function M.root()
  return run_bool_cmd("adb root", function(p)
    return lib.adb_root(c_serial(), p)
  end)
end

function M.unroot()
  return run_bool_cmd("adb unroot", function(p)
    return lib.adb_unroot(c_serial(), p)
  end)
end

function M.remount()
  return run_bool_cmd("adb remount", function(p)
    return lib.adb_remount(c_serial(), p)
  end)
end

function M.reboot(mode)
  mode = mode or ""
  return run_bool_cmd("adb reboot " .. mode, function(p)
    return lib.adb_reboot(c_serial(), mode, p)
  end)
end
...
function M.tcpip(port)
  if not port or port < 1 or port > 65535 then
    vim.notify(
      "[adb-bridge] porta inválida (1..65535)",
      vim.log.levels.ERROR
    )
    return false
  end
  return run_bool_cmd("adb tcpip " .. port, function(p)
    return lib.adb_tcpip(c_serial(), port, p)
  end)
end
...
function M.forward(local_spec, remote_spec)
  return run_bool_cmd(
    ...
    function(p)
      return lib.adb_forward(c_serial(), local_spec, remote_spec, p)
    end
  )
end
...
function M.push(local_path, remote_path)
  return run_bool_cmd(
    ...
    function(p)
      return lib.adb_push(c_serial(), local_path, remote_path, p)
    end
  )
end
...
function M.pull(remote_path, local_path)
  return run_bool_cmd(
    ...
    function(p)
      return lib.adb_pull(c_serial(), remote_path, local_path, p)
    end
  )
end
